...
int main(int argc, char ** argv)
{
    ...
    try {
        ...
        tri2d_sorting_kernel.setArg(0, (cl_ulong)numTris);
        // tri2d_sorting_kernel.setArg(1, d_A); // we will set this later.
        tri2d_sorting_kernel.setArg(2, d_A);
        tri2d_sorting_kernel.setArg(3, d_B);
        tri2d_sorting_kernel.setArg(4, d_C);
        tri2d_sorting_kernel.setArg(5, d_sf);

        for (auto const &b : pointSets) {
            Timer t("Point Test");
            tri2d_sorting_kernel.setArg(1, b.first);
            queue.enqueueNDRangeKernel(tri2d_sorting_kernel, cl::NullRange, cl::NDRange(numTris), cl::NullRange);
